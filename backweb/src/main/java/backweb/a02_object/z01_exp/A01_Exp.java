...
### 2. 생성자와 필드
생성자는 객체를 생성할 때, 사용하는 함수
Person p01 = new Person();
new Person(); ==> 이 부분은 class에서 정의가 되지 않았는 때는
컴파일시 내부적을 default생성자가 만들어져서 호출된다.
이 default 생성자는 하나라도 선언되는 순간 사라진다.
생성자는 오버로딩규칙(갯수, 타입, 순서)에 의해 여러 개를 클래스 안에서 선언할 수 있다.
이 중에 특정한 생성자를 선택해서 객체 생성을 한다.
생성자는 return값이 없고, 주로 필드값을 초기화 할 때, 주로 사용한다.
Book b1 = new Book("java기초", 33000);
==> 호출해서 필드의 title과, price를 할당한다. 
...
ex) 아기가 태어날 기본정보 설정 ==> 생성자.(나이,주민번호,이름,몸무게,키)
    나이, 몸무게,키 계속 변경하는 데이터 ==> 메서드를 통해서 계속 변경 처리..
    Person p01 = new Person("홍길동", 25); //
    p01 = new Person("홍길동", 25); // 객체가 다름
    동일한 객체는 생성자를 한번만 호출하여 설정..
	
	p01.setAge(26); // 위에서 선언한 객체의 속성을 변경(같은 객체)
	
	ps) 생성자는 호출하는 순간 다른 객체가 생성되지만,
	메서드는 호출하면 같은 객체의 속성값만 변경됨..(주의)
